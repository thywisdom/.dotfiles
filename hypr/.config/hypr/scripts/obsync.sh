#!/usr/bin/env bash
# obsync.sh — Git-based Obsidian vault sync (push/pull/rebase)
...
cd "$VAULT_DIR" || {
  notify-send -a "Obsidian Sync" -u critical "Sync Error" "Vault directory not found!"
  exit 1
}
...
git fetch origin main || {
  notify-send -a "Obsidian Sync" -u critical "Network Error" "Failed to connect to remote."
  exit 1
}
